...
const CARD_ROUNDING = 'rounded-[40px] sm:rounded-[50px] md:rounded-[60px]';
    ...
    >
      ...
      <div className="flex flex-col gap-0 max-w-7xl mx-auto">
        {PROJECTS.map((project, i) => (
          <StackCard key={project.number} project={project} index={i} />
        ))}
      </div>
    </section>
  );
...
function StackCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <div ref={containerRef} className="h-[85vh] sticky top-24 md:top-32" style={{ top: `calc(6rem + ${index * 28}px)` }}>
      <motion.div
        style={{ scale }}
        className={`${CARD_ROUNDING} border-2 border-[#D7E2EA] bg-[#0C0C0C] p-4 sm:p-6 md:p-8 h-full flex flex-col overflow-hidden`}
      >
        {/* Top row */}
        ...
        <div className="flex gap-3 sm:gap-4 flex-1 min-h-0">
          ...
          <div className="flex flex-col gap-3 sm:gap-4 w-[40%]">
            <img
              ...
              alt={project.name}
              loading="lazy"
              className={`w-full object-cover ${CARD_ROUNDING}`}
            ...
            <img
              ...
              alt={project.name}
              loading="lazy"
              className={`w-full object-cover flex-1 ${CARD_ROUNDING}`}
          ...
          <div className="w-[60%]">
            <img
              ...
              alt={project.name}
              loading="lazy"
              className={`w-full h-full object-cover ${CARD_ROUNDING}`}
            />
          </div>
        </div>
      </motion.div>
